...
s8 Render_VkResultStr(VkResult r) {
	switch (r) {
...
s8 Render_VkObjectTypeStr(VkObjectType t) {
    switch (t) {
...
s8 Render_VkPhysicalDeviceTypeStr(VkPhysicalDeviceType t) {
	switch (t) {
...
s8 Render_VkQueueFlagsStr(VkQueueFlags f) {
	Array<char> a;
	Array_Init(&a, mem_scratch);
    if (f & VK_QUEUE_GRAPHICS_BIT) { FmtTo(&a, "GRAPHICS | "); }
    if (f & VK_QUEUE_COMPUTE_BIT) { FmtTo(&a, "COMPUTE | "); }
    if (f & VK_QUEUE_TRANSFER_BIT) { FmtTo(&a, "TRANSFER | "); }
    if (f & VK_QUEUE_SPARSE_BINDING_BIT) { FmtTo(&a, "SPARSE_BINDING | "); }
    if (f & VK_QUEUE_PROTECTED_BIT) { FmtTo(&a, "PROTECTED | "); }
    if (f & VK_QUEUE_VIDEO_DECODE_BIT_KHR) { FmtTo(&a, "VIDEO_DECODE | "); }
    if (f & VK_QUEUE_VIDEO_ENCODE_BIT_KHR) { FmtTo(&a, "VIDEO_ENCODE | "); }
	if (a.len > 3) {
		a.len -= 3;
	}
	return s8(a.ptr, a.len);
}

s8 Render_VkFormatStr(VkFormat f) {
	switch (f) {
...
s8 Render_VkColorSpaceStr(VkColorSpaceKHR s) {
	switch (s) {
...
s8 Render_VkPresentModeStr(VkPresentModeKHR m) {
	switch (m) {
...
s8 Render_VkMemoryPropertyFlagsStr(VkMemoryPropertyFlags f) {
	Array<char> a;
	Array_Init(&a, mem_scratch);
    if (f & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) { FmtTo(&a, "DEVICE_LOCAL | "); }
    if (f & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) { FmtTo(&a, "HOST_VISIBLE | "); }
    if (f & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) { FmtTo(&a, "HOST_COHERENT | "); }
    if (f & VK_MEMORY_PROPERTY_HOST_CACHED_BIT) { FmtTo(&a, "HOST_CACHED | "); }
    if (f & VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT) { FmtTo(&a, "LAZILY_ALLOCATED | "); }
    if (f & VK_MEMORY_PROPERTY_PROTECTED_BIT) { FmtTo(&a, "PROTECTED | "); }
    if (f & VK_MEMORY_PROPERTY_DEVICE_COHERENT_BIT_AMD) { FmtTo(&a, "DEVICE_COHERENT | "); }
    if (f & VK_MEMORY_PROPERTY_DEVICE_UNCACHED_BIT_AMD) { FmtTo(&a, "DEVICE_UNCACHED | "); }
    if (f & VK_MEMORY_PROPERTY_RDMA_CAPABLE_BIT_NV) { FmtTo(&a, "RDMA_CAPABLE | "); }
    if (f & VK_QUEUE_VIDEO_ENCODE_BIT_KHR) { FmtTo(&a, "VIDEO_ENCODE | "); }
	if (a.len > 3) {
		a.len -= 3;
	}
...
s8 Render_VkMemoryHeapFlagsStr(VkMemoryHeapFlags f) {
	Array<char> a;
	Array_Init(&a, mem_scratch);
    if (f & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) { FmtTo(&a, "DEVICE_LOCAL | "); }
    if (f & VK_MEMORY_HEAP_MULTI_INSTANCE_BIT) { FmtTo(&a, "MULTI_INSTANCE | "); }
	if (a.len > 3) {
		a.len -= 3;
	}
	return s8(a.ptr, a.len);
