...
TEST("Map") {
	...
	Map<Key, u64> map;
	Map_Init(&map, mem_scratch);
	CHECK(!Map_Find(&map, Key {0, 0}).found);

	Map_Put(&map, Key {0, 0}, (u64)0);
	CHECK_MAP(0, 0x100 | 0, 0);

	Map_Put(&map, Key {1, 0}, (u64)1);
	Map_Put(&map, Key {2, 0}, (u64)2);
	Map_Put(&map, Key {3, 0}, (u64)3);
	...
	Map_Put(&map, Key {4, 4}, (u64)4);
	Map_Put(&map, Key {5, 5}, (u64)5);
	Map_Put(&map, Key {6, 6}, (u64)6);
	Map_Put(&map, Key {7, 7}, (u64)7);
	...
	Map_Put(&map, Key { 9, 1}, (u64)9);
	Map_Put(&map, Key {10, 1}, (u64)10);
	Map_Put(&map, Key {11, 1}, (u64)11);
	...
	Map_Remove(&map, Key {0, 0});
	Map_Remove(&map, Key {1, 0});
	Map_Remove(&map, Key {2, 0});
	if (CHECK_EQ(map.elemsLen, (u64)8)) {
