...
struct Arg {
	...
	union {
		...
		U64 u;
		F64 f;
		struct { const char* data; U64 len; } s;
		const void* p;
	};
};
...
Arg MakeArg(T val) {
	...
	else if constexpr (IsSameType<Underlying, Str>)                { return { .type = ArgType::Str,  .s = { .data = val.data, .len = val.len } }; }
...
struct Str {
	...
	constexpr      Str(const char* s, U64 l) { JC_ASSERT(s || l == 0); data = s; len = l; }
